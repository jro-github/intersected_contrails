...
public class wake_vortex_intersection_v1_for_s3 {
	...
	public wake_vortex_intersection_v1_for_s3(String ConfigFileName_0, String ConfigFileName_1) {
		...
		try {
			...
			eineZeile_0 = file_0.readLine();
			this.fn_output_0 = "records\\results\\s3_"+angle+"d_Ausgabe_LATm50_LON150_vortex_0.txt";
			// Read flight level [m]
			eineZeile_0 = file_0.readLine();
		...
		try {
			...
			// Read the line of invalid result file path;
			eineZeile_1 = file_1.readLine();
			this.fn_output_1 = "records\\results\\s3_"+angle+"d_Ausgabe_LATm50_LON150_vortex_1.txt";
			// Read flight level [m]
			eineZeile_1 = file_1.readLine();
	...
	public void ContrailEvolution() {
		...
		try {
			this.fwDataOut_0.write(
					"t, z, eps, Dh, Dv, sigmah, sigmav, sigmas, rice, Nice, g_t, Qabs_t, Qsca_t, g_s, Qabs_s, Qsca_s, c0_esp, CCS, CCS_rest, V, V_rest, IWC, RHice, Re, v_sed, windshear_s, wind_u, wind_v, wind_w, lat, lon, altitude\n");
			rext_0 = dice_0 / 2. * 1.e6;
			this.fwDataOut_0
					.write(t_0 + ", " + z_0 + ", " +cp_inCE_0.eps + ", " + cp_inCE_0.Dh + ", " + cp_inCE_0.Dv+ ", "+ sigmah_0 + ", " + sigmav_0 + ", " + sigmas_0 + ", " + (dice_0 / 2)
							...
							+ ", " + Ext_0.Calc_Qsca_sol(rext_0, bsol_0) + ", " 
							+ ((int) (c0_esp * 10000) / 10000.) + ", "
							+ CCS_0 + ", " + CCS_rest_0 + ", "
		...
		try {
			this.fwDataOut_1.write(
					"angle, time_diff, t, z, eps, Dh, Dv, sigmah, sigmav, sigmas, rice, Nice, g_t, Qabs_t, Qsca_t, g_s, Qabs_s, Qsca_s, c1_esp, CCS, CCS_rest, V, V_rest, IWC, RHice, Re, v_sed, windshear_s, wind_u, wind_v, wind_w, lat, lon, altitude\n");
			rext_1 = dice_1 / 2. * 1.e6;
			this.fwDataOut_1
					.write((this.angle) + ", " + time_diff + ", " + t_1 + ", " + z_1 + ", " +cp_inCE_1.eps + ", " + cp_inCE_1.Dh + ", " + cp_inCE_1.Dv+ ", " + sigmah_1 + ", " + sigmav_1
		...
		double distance_between_two_contrails = 0;
		...
		do {
			...
			if ((evo_state_0 == false) && (evo_done_0 == false)) {
				...
				if (intersection_check_flag == true) {
					...
					dice_0 = Math.pow(IWC_0 * CCS_0 * this.distance_0 * (1 - intersection_percentage_cal(volumn_inter, CCS_0, this.distance_0)) / (Nice_rest_0 * 917) * 6 / Math.PI, 1 / 3.);
				...
				if ((evo_state_1 == false) && (evo_done_1 == false)) {
					if (c1_initial_full_inside_c0_flag == false) {
						...
						if (intersection_check_flag == true) {
							...
							dice_1 = Math.pow(IWC_1 * CCS_1 * this.distance_1 * (1 - intersection_percentage_cal(volumn_inter, CCS_1, this.distance_1)) / (Nice_rest_1 * 917) * 6 / Math.PI, 1 / 3.);
				...
				if ((intersection_check_flag == false)
					...
					if (angle == 0) { // FIXME
						if (intersection_percentage_cal(volumn_inter, CCSc_1, this.distance_1) == 1.) {
							...
							IWC_1 = 10e-8;
							RHice_1 = 99.9999;
							System.out.println("c1_full_inside_c0_flag is set to true, this scenario 1 can not be calculated in code v_1,");
					...
					if (cp_inter_initialization_flag == false) {
						String fn_output_inter = "records\\results\\s3_"+angle+"d_Ausgabe_LATm50_LON150_vortex_inter.txt";
						...
						try {
							fwDataOut_inter.write(
									"angle, time_diff, t, z, eps, Dh, Dv, sigmah, sigmav, sigmas, rice, Nice, g_t, Qabs_t, Qsca_t, g_s, Qabs_s, Qsca_s, CCS, CCS_esti, V_inter, IWC, RHice, Re, v_sed, windshear_s, wind_u, wind_v, wind_w, lat, lon, altitude\n");
							Ext_inter = new Extinktionseffizienz();
...
//						System.out.println("contrail 1 sigmah_1:" + sigmah_1);
				...
				// if calculate c0 and c1 independently, change Dh and Dv of "this.cp_inter" to "cp_inCE_1" 
				...
				if ((evo_state_1 == false) && (evo_done_1 == false)) {					
					if ((RHice_1 < 100) || (IWC_1 < 10e-8)) {
						System.out.println(
								"CCS and IWC keep the same with the values in the second last loop, which not updated any more!");
						System.out.println(
								"Current RHice_1 = " + RHice_1 + " IWC_1 = " + IWC_1 + ", at t_1 = " + t_1);
						evo_state_1 = true;
					} else {
						...
						if (intersection_check_flag == true) {
							...
							c1_esp = contrail1_exposed_surface_percentage_cal(angle,
									distance_between_two_contrails, sigmav_0, sigmah_0, CCS_0, sigmav_1, sigmah_1,
									CCS_1, this.distance_1);
				...
				if (intersection_check_flag == true) {
					if (c1_initial_full_inside_c0_flag) {
						...
						volumn_inter = intersection_volumn_cal(angle, cp_inCE_0, sigmav_0, sigmah_0, CCS_0, this.cp_inter,
								sigmav_inter, sigmah_inter, CCS_inter, this.distance_0);
				...
				if ((c1_initial_full_inside_c0_flag == false) && (evo_state_1 == false)) {
					try {
						...
						this.fwDataOut_1.write((this.angle) + ", " + time_diff + ", " + t_1 + ", " + z_1 + ", "
								+cp_inCE_1.eps + ", " + cp_inCE_1.Dh + ", " + cp_inCE_1.Dv+ ", "+ sigmah_1 + ", " + sigmav_1 + ", " + sigmas_1 + ", " + (dice_1 / 2) + ", "
								+ Nice_rest_1 + ", " + Ext_1.Calc_g_terr(rext_1, lterr_1) + ", "
								...
								+ ", " + Ext_1.Calc_Qsca_sol(rext_1, bsol_1) + ", " 
								+ ((int) (c1_esp * 10000) / 10000.) + ", "
								+ CCS_1 + ", " + CCS_rest_1 + ", "
								+ (CCS_1 * this.distance_1) + ", " + V_rest_1 + ", " + IWC_1 + ", " + RHice_1 + ", "
								+ Re(dice_1 / 2., SedimentationSpeed(dice_1, cp_inCE_1),
										kinvisc_Suth(gribdata.getPressureFromAltitude(cp_inCE_1.lat, cp_inCE_1.lon,
												cp_inCE_1.altitude), cp_inCE_1.T))
								+ ", " + SedimentationSpeed(dice_1, cp_inCE_1) + ", " + cp_inCE_1.s + ", "
			...
			if (evo_state_0 == false) {
				try {
					rext_0 = dice_0 / 2. * 1.e6;
					this.fwDataOut_0.write(t_0 + ", " + z_0 + ", "+cp_inCE_0.eps + ", " + cp_inCE_0.Dh + ", " + cp_inCE_0.Dv+ ", " + sigmah_0 + ", " + sigmav_0 + ", " + sigmas_0 + ", "
							+ (dice_0 / 2) + ", " + Nice_rest_0 + ", " + Ext_0.Calc_g_terr(rext_0, lterr_0) + ", "
							+ Ext_0.Calc_Qabs_terr(rext_0, lterr_0) + ", " + Ext_0.Calc_Qsca_terr(rext_0, lterr_0)
							+ ", " + Ext_0.Calc_g_sol(rext_0, bsol_0) + ", " + Ext_0.Calc_Qabs_sol(rext_0, bsol_0)
							+ ", " + Ext_0.Calc_Qsca_sol(rext_0, bsol_0) + ", " 
							+ ((int) (c0_esp * 10000) / 10000.) + ", "
			...
			if ((intersection_check_flag) && (evo_state_inter == false)) {
				try {
					...
					fwDataOut_inter.write((this.angle) + ", " + time_diff + ", " + t_inter + ", " + z_inter + ", "
							+this.cp_inter.eps + ", " + this.cp_inter.Dh + ", " + this.cp_inter.Dv+ ", "+ sigmah_inter + ", " + sigmav_inter + ", " + sigmas_inter + ", " + dice_inter / 2. + ", "
							...
							+ Ext_inter.Calc_Qsca_sol(rext_inter, bsol_inter) + ", " + CCS_inter + ", " + CCS_inter_esti
							+ ", " + volumn_inter + ", " + IWC_inter + ", " + RHice_inter + ", "
							+ Re(dice_inter / 2., SedimentationSpeed(dice_inter, this.cp_inter),
									kinvisc_Suth(gribdata.getPressureFromAltitude(this.cp_inter.lat, this.cp_inter.lon,
											this.cp_inter.altitude), this.cp_inter.T))
							+ ", " + SedimentationSpeed(dice_inter, this.cp_inter) + ", " + this.cp_inter.s + ", "
